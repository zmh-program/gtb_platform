...
export async function getPlayerStats(username: string, apiKey: string): Promise<PlayerStats> {
  try {
    ...
    const response = await fetch(`https://api.hypixel.net/v2/status?key=${apiKey}&uuid=${uuid}`);
    if (!response.ok) {
      return {
        success: false,
    ...
    // Get player data
    const playerResponse = await fetch(`https://api.hypixel.net/v2/player?key=${apiKey}&uuid=${uuid}`);
