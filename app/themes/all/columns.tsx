...
export const createColumns = (
  ...
  const baseColumns: ColumnDef<ThemeRow>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="h-full flex items-center px-3 font-medium text-[#eee] truncate">
          {row.getValue("theme")}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            className={cn(
              "h-full flex items-center px-3 font-bold text-[10px] truncate",
              shortcut ? "text-white" : "text-[#444]",
            )}
          >
            {shortcut || "-"}
          </div>
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="h-full flex items-center gap-2 px-3 py-1">
            <div
              className="flex flex-col text-[10px] leading-tight min-w-0"
              title={text}
            >
  ...
  const langColumns: ColumnDef<ThemeRow>[] = languageKeys.map((lang) => ({
    ...
    cell: ({ row }) => {
      ...
      if (!t)
        return (
          <div className="h-full flex items-center px-3 text-[#333]">-</div>
        );

      const approved = t.is_approved as any;
      const isApproved =
        approved === true || approved === 1 || approved === "true";
